...
the whole `publish` job, which is the safe way to test changes to the release
pipeline. Since packaging lives in `publish` (see below), a dispatch run leaves
the payload as a staged directory in the `dist-linux`/`dist-macos` artifacts
rather than as finished archives.
...
**Packaging happens once, in `publish`.** The build jobs only *stage* a
directory (binary + `scripts/` + LICENSE + README) and upload that; `publish`
turns each staged directory into the `.tar.gz` (Linux) / `.zip` (macOS) that
`docs/src/installation.md` links to. Archiving in the build jobs instead would
make every Actions-tab download a GitHub zip wrapped around a tar.gz, since
`upload-artifact` always zips. Two consequences of routing the payload through
an artifact: `upload-artifact` does not preserve file modes, so `publish`
re-applies `chmod +x`; and the macOS zip is no longer the one submitted to
`notarytool`, which is fine because the ticket is bound to the signed binary and
served from Apple's CDN (nothing is stapled to the archive).
...
Everything below lives in the **`release` environment**, not at repository level:
Settings → Environments → `release`. The three jobs that consume it
(`build-macos`, `publish`, `docs`) therefore declare `environment: release`;
without that line every `secrets.*`/`vars.*` reference silently resolves to the
empty string and the guards in those jobs fail with "Missing …".

The environment currently has no protection rules, so a `v*` tag deploys without
waiting for an approval. Adding required reviewers there is the place to change
that — it needs no workflow edit.
...
| `CLOUDFRONT_DISTRIBUTION_ID` | docs deploy | Not actually sensitive, but it is stored as a secret, so the workflow reads it via `secrets.` |
...
Settings → Environments → `release` → **Environment variables**
...
**CRLF trap for multi-line values.** The web UI stores anything typed into a
multi-line value box with `\r\n` endings, and shell word splitting breaks on
IFS (space/tab/newline) but *not* on `\r`. A naive `for x in $VAR` therefore
hands every element except the last a trailing carriage return — for
`ALL_APPLE_CERTS` that surfaced as `curl: (3) URL using bad/illegal format`.
The macOS job strips the CRs before splitting; do the same in any new step that
iterates over a multi-line variable. (`base64 --decode` is safe: it ignores CR.)
