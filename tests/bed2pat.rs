...
fn can_create_pat() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(p12_line.is_some(), "Could not find read starting at position 12");
    Ok(())
}
