...
fn report_all() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let total: u32 = output_str.lines()
                               .map(|elem| {
                                   let row_elems: Vec<&str> = elem.split("\t").collect();
                                   row_elems[9].parse::<u32>().unwrap_or_default()
                               })
                               .sum();
...
fn restrict_to_chromosome() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let total_mod: usize = output_str.lines()
                                     .map(|elem| {
                                         let row_elems: Vec<&str> = elem.split("\t").collect();
                                         row_elems[11].split(",").collect::<Vec<&str>>().len()
                                     })
                                     .sum();
...
fn correct_pos_with_skips() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let roi =
        output_str.lines()
                  .filter(|l| {
                      predicate::str::contains("NB502094:69:HN2H2BGX5:2:22305:19777:14996").eval(l)
                  })
...
fn correct_data_in_random_region_2() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let some_row =
        output_str.lines()
                  .filter(|l| {
                      predicate::str::contains("NB502094:69:HN2H2BGX5:4:11501:13313:12598").eval(l)
                  })
                  .last()
                  .unwrap_or_default();
...
fn reports_snps_at_cpg() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let some_row =
        output_str.lines()
                  .filter(|l| {
                      predicate::str::contains("NB502094:69:HN2H2BGX5:3:22406:24759:2699").eval(l)
                  })
                  .last()
                  .unwrap_or_default();
    let elems = some_row.split("\t").collect::<Vec<&str>>();
...
fn does_not_report_deleted_positions() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let some_row =
        output_str.lines()
                  .filter(|l| {
                      predicate::str::contains("NB502094:69:HN2H2BGX5:3:22506:13203:14643").eval(l)
                  })
                  .last()
                  .unwrap_or_default();
    let elems = some_row.split("\t").collect::<Vec<&str>>();
...
fn correctly_reports_read_length() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    some_row =
        output_str.lines()
                  .filter(|l| {
                      predicate::str::contains("NB502094:69:HN2H2BGX5:4:22504:24050:9422").eval(l)
                  })
                  .last()
                  .unwrap_or_default();
