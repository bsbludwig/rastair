...
- **M-bias position parsing is bounded by 250,000-row chunks.** Never concatenate every encoded position string in a whole `(chr, read_pair, strand)` group: both that string and `scan()`'s integer result scale with the largest chromosome/group and can exceed cluster memory on deep data. `position_counts()` chunks rows before data.table grouping, passes filtered character vectors directly to `scan(text=encoded, ...)`, and accumulates mod/unmod counts together. It operates on the existing in-memory table, which remains available to V-bias without rereading the source. On 4,259,458 records from `5042_HCC` chr1, the complete no-V-bias report retained byte-identical HTML and cutoffs while improving from 20.66 s / 1,976,472 KB peak RSS to 17.91 s / 1,705,644 KB. The direct `scan()` call alone parsed modified positions in 1.08 s versus 1.96 s with `paste`.
- **A deduplicated per-read BED row is the V-bias alignment identity.** Do not assign `alignment_id` with a full-table `.GRP` over read names and coordinates at the end of `transform_data`; that groups tens of millions of mostly unique strings before V-bias begins and adds a chromosome-scale workspace while the report still appears to be in the M-bias cell. `expand_positions()` already computes the source row indices needed to repeat metadata, so carry those indices as `alignment_id` and rank observations within them. On 4,259,428 rows, the former grouping took 3.27 s while assigning row identity itself was effectively instantaneous. A 200-iteration stress test that ran the chunked M-bias parser over the equivalent of 50 million rows peaked at only 198 MB with no growth, confirming that repeated `scan()` calls do not retain per-chunk working memory; investigate full-table groupings/copies when `transform_data` grows instead.
- **`--region chr1` is required for a chr1-only report.** Supplying a whole-genome BED and reference does not infer chromosome scope. Check the printed parameter list: `region: NA` selects `bgzip -cd` and loads the entire BED, whereas `region: "chr1"` selects `tabix -h ... chr1`. On `5042_HCC`, the current M-bias-only `transform_data` completed actual chr1 in 95 s at 12,438,664 KB peak RSS under a 60 GB R heap limit; a command described as chr1-only but missing `--region chr1` instead attempted the whole 7.9 GB compressed file and exceeded a 100 GB Slurm limit.
