...
pub struct VariantCount {
    ...
    pub pos: u64,
    pub ref_base: u8, 
    pub forward: NucleotideCount,
    pub reverse: NucleotideCount,
}
...
impl VariantCount {
    pub fn new() -> Self {
        ...
        VariantCount { contig: String::new(), pos: 0, ref_base: 0, forward: fw, reverse: rv }
    }
}
...
impl fmt::Display for VariantCount {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(f, "{}:{} ({})\nFW\t{}\nRV\t{}", self.contig, self.pos, char, self.forward, self.reverse)
    }
}
...
impl <P: AsRef<Path> + std::fmt::Debug> VariantCounterConfig<P> {
    pub fn with_paths(fasta_path: P, bam_path: P) -> Result<Self> {
        let v = VariantCounterConfig {
            ...
            max_depth: MAX_DEPTH,
            chunk_size: 10000,
            required_flags: 3, // read paired (0x1) & read mapped in proper pair (0x2)
            excluded_flags: 3852,
        };
        Ok(v)
    }
...
impl <P: AsRef<Path> + std::fmt::Debug> Iterator for VariantCounter<P> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        */
        self.bam
        .fetch((&segment.contig[..], segment.start, segment.stop))
        .expect((format!("Error fetching {}", &segment)).as_str());
        ...
        for pileups in pileup_iterator {
            ...
            if pileup_pos == this_position.pos_in_contig() {
                                                ...
                                                .filter(filter_closure) 
                {
                    ...
                    let nuc_counts = {
                        if alignment.record().flags() & 0x10 == 0 {
                            &mut var_count.forward
                        } else {
                            &mut var_count.reverse
                        }
                    };
