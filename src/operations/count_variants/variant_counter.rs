...
impl VariantCounterConfig
{
    ...
    {
        let v = VariantCounterConfig { bam_path: bam_path.as_ref().to_owned(),
                                       min_mapq: 1,
                                       min_baseq: 10,
                                       max_depth: MAX_DEPTH,
                                       required_flags: FLAGS.is_paired
                                                       | FLAGS.is_properly_paired,
                                       ...
                                       ot_mask: ReadMaskSetting { r1: ReadMask(0, 0),
                                                                  r2: ReadMask(0, 0) },
                                       ob_mask: ReadMaskSetting { r1: ReadMask(0, 0),
                                                                  r2: ReadMask(0, 0) },
                                       htslib_threads: 0,
                                       region: None,
...
impl VariantCounter
{
    ...
    fn generate_alignment_filter<'a>(config: &'a VariantCounterConfig)
                                     -> impl Fn(&Alignment<'a>) -> bool
    {
        let filter_closure = |alignment: &Alignment| -> bool {
            ...
            let read_pair_orientation =
                record.read_pair_orientation_lenient(config.exclude_ambiguous);
    ...
    /// positions
    pub fn count_variants_in_segment(&mut self,
                                     segment: SequenceSegment)
                                     -> Option<Vec<VariantCount>>
    {
         ...
         */
        match self.bam.fetch((segment.region.contig.as_bytes(),
        ...
        let mut read_hash: HashMap<Vec<u8>, (u8, u8), FxBuildHasher> =
            HashMap::with_capacity_and_hasher(self.config.max_depth as usize,
        ...
        'pileup_loop: for pileups in pileup_iterator {
            ...
            'alignment_loop: for alignment in pileup.alignments().filter(filter_closure) {
                ...
                debug!("Processing pos {} in read {} with base {} and qual {} at col {}",
                       pos,
                ...
                if !self.config.keep_overlaps {
                    if let Some(pos_tuple) = read_hash.get(record.qname()) {
                        trace!("Found overlapping read pair {} at pos {} with bases {} vs {}",
                               std::str::from_utf8(record.qname()).unwrap_or_default(),
                               pos,
                               char::from_u32(base as u32).unwrap_or_default(),
                               char::from_u32(pos_tuple.0 as u32).unwrap_or_default());
